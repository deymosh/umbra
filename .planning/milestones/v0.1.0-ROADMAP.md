...
- [x] **Phase 4: Version Consistency & v0.1.0 Release Prep** - One true version, a dated changelog, and a release one push away (completed 2026-09-06)
...
**Success Criteria** (what must be TRUE):
  ...
  4. ~~A `v0.1.0` git tag exists locally and is confirmed absent from the remote (`git ls-remote --tags origin` shows no `v0.1.0`); pushing it is left as an explicit user action.~~ **Descoped by explicit user direction (2026-09-06):** the tag itself is not created as part of Phase 4 or this milestone's close — the user creates it themselves, separately, whenever they choose. `.claude/skills/umbra-release/SKILL.md` (criterion 5) documents the exact command for when that happens.
...
- [x] 04-03-PLAN.md — The umbra-release runbook skill with its confirm-before-push gate (SKILL-01). REL-03 (the local v0.1.0 tag) was descoped by explicit user direction and is not part of this plan's completed scope — see 04-03-SUMMARY.md.
...
| 4. Version Consistency & v0.1.0 Release Prep | 3/3 | Complete    | 2026-09-06 |
