...
- LOG-7's ViewModel-wiring gap (`ProfileViewModel`/`ThreadViewModel` `combine(futureEventRecheckTicker())` wiring untested) — whether it's worth a new wiring-level test beyond the existing ticker/tolerance coverage is left to planning discretion once those ViewModels' testability is assessed. At minimum, add the cheap `EventModelBehaviorTest.kt`-style zero-tolerance-default assertion the research recommended.

### Third disposition — source-read-verified entries (Group D)
- **D-09:** 03-RESEARCH.md identified 6 entries (LOG-18, LOG-20, LOG-28, LOG-39, LOG-51, LOG-54) as structurally unassertable by any unit test — each is a one-line logging-visibility fix behind a non-injected `private val logger = UmbraLog.tag(TAG)`, not a constructor-injected `UmbraLogger`, so no test can intercept the call. These are NOT genuinely "needs a running app" the way LOG-2/3/13/26 are — they're confirmable by reading the current source. User-confirmed resolution: treat "verified by direct source read, cited by file:line" as a third valid disposition alongside test-evidence and needs-eyeball. These 6 entries move to `docs/DONE.md` with a rationale line in place of a `**Validated:**` test citation, e.g. "Verified by direct source read — `logger` is not constructor-injected in this class, so no unit test can assert the logging call; confirmed by inspection of `<file>:<line>`." Do not force them into `KNOWN_ISSUES.md`'s emulator-pending bucket, and do not silently drop the distinction by treating them as ordinary test-backed DONE entries either — the rationale line must make clear no test exists. — **Reversibility:** reversible — a stronger citation (e.g. after LOG-44's architectural work adds an interface seam) can replace the source-read rationale later without re-touching production code.

### Group C entries stay in KNOWN_ISSUES.md, cross-referenced (not deferred like LOG-44)
- **D-10:** LOG-30, LOG-38, LOG-49, LOG-52 all live inside `NostrSessionManager` and hit the identical architectural wall D-08 excluded LOG-44 for (concrete `TorRuntimeManager`/`BackfillAnchorStore` constructor dependencies, no interface seam — a unit test is currently impossible, not merely absent). User-confirmed resolution: unlike LOG-44 (a pre-existing coverage gap with no bug attached), these four are real, deployed, working fixes — there is nothing wrong with the fix itself, only no way to unit-test it yet. Keep all four in `docs/KNOWN_ISSUES.md` at `fix applied — needs on-device validation`, and add one shared note across all four cross-referencing LOG-44's architectural blocker, so a future reader doesn't attempt an isolated `NostrSessionManagerTest.kt` before that interface-seam work happens. Do not move these to `docs/TODO.md`. — **Reversibility:** reversible — once LOG-44's architectural work lands, these four can be revisited and reclassified without any change needed here.
