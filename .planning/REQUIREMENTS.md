...
- [x] **VALID-07**: Validate LOG-11 — ticker's immediate-first-emission regression fix
...
- [x] **VALID-12**: Validate LOG-19 — NIP-09 "a"-tag deletions never take effect for a non-owned author's cached addressable event
- [x] **VALID-13**: Validate LOG-20 — Silent empty catch block during `clearAllData()`'s wipe sequence
- [x] **VALID-14**: Validate LOG-21 — `snapshotEmitJob` is a plain, unsynchronized field mutated from concurrent coroutines
- [x] **VALID-15**: Validate LOG-22 — ProfileViewModel.deleteEvent removes a note from the visible list before Amber confirms the delete, with no rollback
- [x] **VALID-16**: Validate LOG-23 — FeedViewModel.muteUser's local-filter mute mirror is dead code
- [x] **VALID-17**: Validate LOG-24 — FeedViewModel.muteUser/togglePin discard the mute/pin write's success/failure result
...
- [x] **VALID-21**: Validate LOG-29 — RelayCrudCoordinator's per-role enable-flag setters can lose a concurrent update to the same relay
...
- [x] **VALID-23**: Validate LOG-31 — RelayCrudCoordinator.setDmEnabled marks the DM relay list dirty even when the enable is rejected
- [x] **VALID-24**: Validate LOG-34 — Logger.e() leaks the raw, unscrubbed Throwable via Android's own stack-trace formatting, bypassing LogScrubber entirely
...
- [x] **VALID-28**: Validate LOG-40 — EventIngestCache.scheduleInsert's cancel-and-replace ordering lets the old and new debounce jobs run concurrently
- [x] **VALID-29**: Validate LOG-41 — EventIngestCache.cacheRepostTarget skips the replaceable-event supersede bookkeeping ingest() enforces for the same slot
...
- [x] **VALID-31**: Validate LOG-46 — InteractionActionsCoordinator.mirrorMuteIntoActiveFilter used stdlib runCatching, swallowing CancellationException
- [x] **VALID-32**: Validate LOG-47 — RelayCrudCoordinator.saveRelay's merge branch based its OR-merged write on a stale pre-lock snapshot
...
- [x] **VALID-36**: Validate LOG-53 — RelayCrudCoordinator.saveRelay's new-relay/URL-collision decision still read the throttled state.value.relays mirror
...
| VALID-07 | Phase 3 | Complete |
| VALID-08 | Phase 3 | Complete |
| VALID-09 | Phase 3 | Complete |
| VALID-10 | Phase 3 | Complete |
| VALID-11 | Phase 3 | Complete |
| VALID-12 | Phase 3 | Complete |
| VALID-13 | Phase 3 | Complete |
| VALID-14 | Phase 3 | Complete |
| VALID-15 | Phase 3 | Complete |
| VALID-16 | Phase 3 | Complete |
| VALID-17 | Phase 3 | Complete |
| VALID-18 | Phase 3 | Complete |
| VALID-19 | Phase 3 | Complete |
| VALID-20 | Phase 3 | Complete |
| VALID-21 | Phase 3 | Complete |
| VALID-22 | Phase 3 | Complete |
| VALID-23 | Phase 3 | Complete |
| VALID-24 | Phase 3 | Complete |
| VALID-25 | Phase 3 | Complete |
| VALID-26 | Phase 3 | Complete |
| VALID-27 | Phase 3 | Complete |
| VALID-28 | Phase 3 | Complete |
| VALID-29 | Phase 3 | Complete |
| VALID-30 | Phase 3 | Complete |
| VALID-31 | Phase 3 | Complete |
| VALID-32 | Phase 3 | Complete |
